...
public class FaceServer {
    ...
    public boolean registerNv21(Context context, byte[] nv21, int width, int height, FaceInfo faceInfo, String name) {
        synchronized (this) {
            ...
            //特征提取
            FaceFeature faceFeature = getFaceFeature(nv21, width, height,faceInfo);
            if (faceFeature==null) {
                Log.e(TAG, "registerNv21: extractFaceFeature failed , code is null");
                return false;
            } else {
                ...
                try {
                    ...
                    for(FaceRegisterInfo fi:faceRegisterInfoList){
                        if(fi.getName().equals(userName)){
                            faceRegisterInfoList.remove(fi);
                        }
                    }
...
//            }
     ...
     * @return 是否注册成功
     */
    public boolean registerNv21(Context context, byte[] nv21, int width, int height,FaceFeature faceFeature, FaceInfo faceInfo, String name) {
        synchronized (this) {
            if (faceEngine == null || context == null || nv21 == null || width % 4 != 0 || nv21.length != width * height * 3 / 2) {
                Log.e(TAG, "registerNv21: invalid params");
                return false;
            ...
            //特征提取
            if(faceFeature==null) {
                faceFeature = getFaceFeature(nv21, width, height, faceInfo);
            }
            if (faceFeature==null) {
                Log.e(TAG, "registerNv21: extractFaceFeature failed , code is null");
                return false;
     ...
     * @return 是否注册成功
     */
    public FaceFeature getFaceFeature(byte[] nv21, int width, int height, FaceInfo faceInfo) {
        synchronized (this) {
            FaceFeature faceFeature = new FaceFeature();
            //特征提取
            int code = faceEngine.extractFaceFeature(nv21, width, height, FaceEngine.CP_PAF_NV21, faceInfo, faceFeature);
     ...
     * @return 是否注册成功
     */
    public Bitmap getFaceBitmap(byte[] nv21, int width, int height, FaceInfo faceInfo) {
        synchronized (this) {
            FaceFeature faceFeature = new FaceFeature();
            //特征提取
            Rect cropRect = getBestRect(width, height, faceInfo.getRect());
