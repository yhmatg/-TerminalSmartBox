...
public class EkeyServerbak {
    ...
    private static EkeyServerbak eKeyServer;
    private static Timer timer;
    EkeyStatusChangeListener ekeyStatusChangeListener;
    private static long lastRevTime;

    public static EkeyServerbak getInstance() {
        if (eKeyServer == null) {
            synchronized (EkeyServerbak.class) {
                if (eKeyServer == null) {
                    eKeyServer = new EkeyServerbak();
                    eKeyServer.init();
                }
            }
        }
    ...
    final SerialRead.ReadDataListener revListener = new SerialRead.ReadDataListener() {
        ...
        public void onReadData(byte[] data) {
            ...
            //7E0101000000020016E3257E7E   7E010100000002 0016 E325 7E7E
//            if(data.length<9 || data[0]!=(byte)0x7E || data[7]!=0x00|| (data[8]!=0x16&& data[8]!=0x05)){//处理开锁或者查询状态返回
//                return;
//            }
            //处理开锁
//            if(data[8]==0x05){
...
//                    ekeyStatus=EkeyStatus.OPEN;
//                    if(ekeyStatusChangeListener!=null){
//                        ekeyStatusChangeListener.onOpenSuccess(true);
//                    }
//                }else{
//                    if(ekeyStatusChangeListener!=null){
//                        ekeyStatusChangeListener.onOpenSuccess(false);
//                    }
//                }
//                return;
//            }
    ...
    private void init() {
        ...
        timer.schedule(timerTask, 1000, ekey_check_period);
        dataBytes = new byte[64];
    }
